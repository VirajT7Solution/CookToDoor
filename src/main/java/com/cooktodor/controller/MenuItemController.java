...
import com.cooktodor.dto.MenuItemDtos;
...
import com.cooktodor.model.Category;
import com.cooktodor.model.Image;
...
import com.cooktodor.service.ImageService;
import com.cooktodor.service.MenuItemService;
    ...
    private MenuItemDtos.Response toResponse(MenuItem item) {
        ...
        if (item.getImages() != null && !item.getImages().isEmpty()) {
            java.util.List<String> base64List = item.getImages().stream()
                    .map(com.cooktodor.model.Image::getBase64Data)
                    .toList();
            java.util.List<String> fileTypeList = item.getImages().stream()
                    .map(com.cooktodor.model.Image::getFileType)
                    .toList();
